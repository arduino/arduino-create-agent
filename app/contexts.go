...
// InfoManageV1Context provides the manage_v1 info action context.
type InfoManageV1Context struct {
	context.Context
	*goa.ResponseData
	*goa.RequestData
}

// NewInfoManageV1Context parses the incoming request URL and body, performs validations and creates the
// context used by the manage_v1 controller info action.
func NewInfoManageV1Context(ctx context.Context, r *http.Request, service *goa.Service) (*InfoManageV1Context, error) {
	var err error
	resp := goa.ContextResponse(ctx)
	resp.Service = service
	req := goa.ContextRequest(ctx)
	req.Request = r
	rctx := InfoManageV1Context{Context: ctx, ResponseData: resp, RequestData: req}
	return &rctx, err
}

// OK sends a HTTP response with status code 200.
func (ctx *InfoManageV1Context) OK(r *ArduinoAgentManageInfo) error {
	if ctx.ResponseData.Header().Get("Content-Type") == "" {
		ctx.ResponseData.Header().Set("Content-Type", "application/vnd.arduino.agent.manage.info+json")
	}
	return ctx.ResponseData.Service.Send(ctx.Context, 200, r)
}
